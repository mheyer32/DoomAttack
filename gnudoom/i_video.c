...
/*	rc=intuimessage->Code&(~IECODE_UP_PREFIX);*/
    ...
    switch(taste)
    {
      case 79:	rc = KEY_LEFTARROW;	break;
      case 78:	rc = KEY_RIGHTARROW;	break;
      case 77:	rc = KEY_DOWNARROW;	break;
      case 76:	rc = KEY_UPARROW;	break;
      case 69:	rc = KEY_ESCAPE;	break;
      case 68:	rc = KEY_ENTER;		break;
      case 66:	rc = KEY_TAB;		break;
      case 80:	rc = KEY_F1;		break;
      case 81:	rc = KEY_F2;		break;
      case 82:	rc = KEY_F3;		break;
      case 83:	rc = KEY_F4;		break;
      case 84:	rc = KEY_F5;		break;
      case 85:	rc = KEY_F6;		break;
      case 86:	rc = KEY_F7;		break;
      case 87:	rc = KEY_F8;		break;
      case 88:	rc = KEY_F9;		break;
      case 89:	rc = KEY_F10;		break;

/* "~" */
	  case  0:  rc = KEY_F12;		break;
	  
/* NUMERICPAD * */
      case 93:	rc = KEY_F11;		break;
...
/*	  case 61:
      ...
      case 65:	rc = KEY_BACKSPACE; break;
      
      case 70:	rc = KEY_DEL; break;
      
      case 95:  rc = KEY_HELP; break;

/* HELP*/
      case 92:	rc = KEY_PAUSE;		break;

/*      case XK_KP_Equal:*/
/* =*/
      case 12:	rc = KEY_EQUALS;	break;

      case 74:	rc = KEY_MINUS;		break;

      case 96:	rc = KEY_LSHIFT;	break;
      
      case 97:  rc = KEY_RSHIFT;	break;
      
      case 99: rc = KEY_RCTRL; break;
	
      case 100:  rc = KEY_LALT; break;

      case 101:  rc = KEY_RALT; break;
      
      case 102:	 rc = KEY_LAMIGA; break;
      
      case 103:  rc = KEY_RAMIGA; break;
      
      case 90:
      	 if (DoNumericPad) rc = KEY_NUMOPEN; else rc = '(';
      	 break;
      
      case 91:
      	 if (DoNumericPad) rc = KEY_NUMCLOSE; else rc = ')';
      	 break;
      	 
      case 61:
      	 if (DoNumericPad) rc = KEY_NUM7; else rc = '7';
      	 break;
      	 
      case 62:
      	 if (DoNumericPad) rc = KEY_NUM8; else rc = '8';
      	 break;
      	 
      case 63:
      	 if (DoNumericPad) rc = KEY_NUM9; else rc = '9';
      	 break;
      	 
      case 45:
      	 if (DoNumericPad) rc = KEY_NUM4; else rc = '4';
      	 break;
      	 
      case 46:
      	 if (DoNumericPad) rc = KEY_NUM5; else rc = '5';
      	 break;
      	 
      case 47:
      	 if (DoNumericPad) rc = KEY_NUM6; else rc = '6';
      	 break;
      
      case 29:
      	 if (DoNumericPad) rc = KEY_NUM1; else rc = '1';
      	 break;
      	 
      case 30:
      	 if (DoNumericPad) rc = KEY_NUM2; else rc = '2';
      	 break;
      	 
      case 31:
      	 if (DoNumericPad) rc = KEY_NUM3; else rc = '3';
      	 break;
      	 
      case 15:
      	 if (DoNumericPad) rc = KEY_NUM0; else rc = '0';
      	 break;
      	 
      case 60:
      	 if (DoNumericPad) rc = KEY_NUMP; else rc = '.';
      	 break;
      	 
      case 67:
			 if (DoNumericPad) rc = KEY_NUMENTER; else rc = KEY_ENTER;
			 break;
...
static void InitKeys(void)
{
	...
	for (i=0;i<(sizeof(keyarray) / sizeof(int *));i++)
	{
		for (i2=0;i2<(sizeof(cmpkeyarray) / sizeof(int));i2++)
		{
			...
			{
				DoNumericPad=TRUE;break;
			}
		}
		if (DoNumericPad) break;
	}
...
void I_ShutdownGraphics(void)
{
  ...
  if (RemapTable) FreeVec(RemapTable);
  if (RemapBuffer) FreeVec(RemapBuffer);
  if (ObtainTable)
  {
  		I_FreeAllocedPens();
		FreeVec(ObtainTable);
  ...
  if (Blitting && DoDoubleBuffer) Wait(SIGBREAKF_CTRL_F);
  ...
  if (DoomSig != -1) FreeSignal(DoomSig);
  ...
  if (!DoGraffiti)
  {
	  ...
	  if (screenbuffer1) FreeScreenBuffer(screen,screenbuffer1);
	  if (screenbuffer2) FreeScreenBuffer(screen,screenbuffer2);
  ...
  {
  	 WaitBlit();
  	 if (DoMMU1) mmu_mark(Planes1,(REALSCREENWIDTH / 8 * REALSCREENHEIGHT * Depth + 4095) & (~0xFFF),oldmmu_planes1,SysBase);
  ...
  {
  	  WaitBlit();
	  if (DoMMU1) mmu_mark(Planes2,(REALSCREENWIDTH / 8 * REALSCREENHEIGHT * Depth + 4095) & (~0xFFF),oldmmu_planes2,SysBase);
  ...
  if (MausObj) DisposeObject(MausObj);
  if (MausBM) FreeBitMap(MausBM);
...
static void InactiveSleep(void)
{
	...
	if (!DoGraffiti)
	{
		...
		Move(&viewrastport,(REALSCREENWIDTH-i)/2,(REALSCREENHEIGHT-viewrastport.TxHeight)/2+viewrastport.TxBaseline);
	...
	while ((intuimessage=(struct IntuiMessage *)GetMsg(window->UserPort)))
	{
		 ...
		 switch(intuimessage->Class)
	    {
	      ...
	      case IDCMP_MOUSEBUTTONS:
				...
				if (ActQualifier & IEQUALIFIER_LEFTBUTTON) event.data1 |= 1;
				if (ActQualifier & IEQUALIFIER_MIDBUTTON) event.data1 |= 2;
				if (ActQualifier & IEQUALIFIER_RBUTTON) event.data1 |= 4;
...
/*			   	}*/
	...
	} else if (DoJoyPad)
	{
	   ...
	   if (JoyPos == PreviousJoyPos) return;

		if ((JoyPos & JP_TYPE_MASK) == JP_TYPE_NOTAVAIL) return;

	    joyevent.type = ev_joystick;
	    joyevent.data1 = joyevent.data2 = joyevent.data3 = 0;

	    if (JoyPos & JPF_BUTTON_RED) joyevent.data1 |= 1; else joyevent.data1 &= ~1;
...
static void ShowFPS(void)
{
	...
	s[2]=LongRest(fps,10)+'0';fps=LongDiv(fps,10);
	s[1]=LongRest(fps,10)+'0';fps=LongDiv(fps,10);
...
void I_FinishUpdate (void)
{
	...
	if (DoFPS && DoBlitToScreen) ShowFPS();
	...
	if (DoBlitToScreen)
	{
		if (InWindow) RemapScreen(screens[0],RemapBuffer,REALSCREENWIDTH * REALSCREENHEIGHT);

		if (DoC2P)
		{
			C2P->Chunky2Planar(InWindow ? RemapBuffer : screens[0],Planes);
			if (C2PIsFlipping) Blitting=TRUE;
			...
			if (OS31)
			{
				WriteChunkyPixels(
					&renderrastport,
					BlitX,BlitY,
					BlitX+REALSCREENWIDTH-1,BlitY+REALSCREENHEIGHT-1,
					InWindow ? RemapBuffer : screens[0],
					REALSCREENWIDTH);
			} else {
				WritePixelArray8(
					&renderrastport,
	...
	if (DoDoubleBuffer)
	{
		if (!C2PIsFlipping || !DoBlitToScreen) Signal(FlipTask,FlipMask);
	}
}
...
static void I_FreeAllocedPens(void)
{
	...
	if (PensAlloced)
	{
		for (i=0;i<14*256;i++)
		{
			if (ObtainTable[i] != -1) ReleasePen(colormap,ObtainTable[i]);
		}
	}
}
...
static void I_InitRemap(UBYTE *palette)
{
	...
	for (i=0;i<14*256;i++)
	{
		...
		col=ObtainBestPen(colormap,RGB32(r),RGB32(g),RGB32(b),
...
void I_InitGraphics(void)
{
   ...
   static int		firsttime=1;
	...
	if (cputype >= 68040)
	{
		if ((usemmu & 1) || M_CheckParm ("-mmu")) DoMMU1=TRUE;
		if ((usemmu & 2) || M_CheckParm ("-mmu2")) DoMMU2=TRUE;
	}
...
/*    signal(SIGINT, (void (*)(int)) I_Quit);*/
			...
			if ((JoyIO=(struct IOStdReq *)CreateIORequest(JoyMP,sizeof(struct IOStdReq))))
			{
				if (!OpenDevice("gameport.device",joyport,(struct IORequest *)JoyIO,0))
				{
				    ...
				    if (JoyType == GPCT_NOCONTROLLER)
				    {
				    	...
				    	Permit();
				    	CloseDevice((struct IORequest *)JoyIO);
				    	DeleteIORequest((struct IORequest *)JoyIO);JoyIO=0;
				    	DeleteMsgPort(JoyMP);JoyMP=0;
	...
	if (M_CheckParm("-fps")) DoFPS=TRUE;
	if (M_CheckParm("-rendercontrol")) NoRender=TRUE;

	NoRangeCheck=(boolean)ConfigNoRangeCheck;
	if (M_CheckParm("-norangecheck")) NoRangeCheck=TRUE;
	...
	if (InputBase)
	{
		if (StartQualifier & (IEQUALIFIER_LSHIFT | IEQUALIFIER_RSHIFT)) modeid=INVALID_ID;
	}
	...
	if (DoFastRemap)
		...
		if (!RemapTable || !ObtainTable || !RemapBuffer) I_Error("I_Video: Out of memory (Remaptable/ObtainTable/RemapBuffer)");
		...
		if (!screen) I_Error("I_Video: Can't lock screen!");

	} else if ((modeid == 0xFFFFFFFC) ||
				  (modeid == 0xFFFFFFFB))
	{
		...
		if (InWindow) I_Error("I_Video: Window mode is not possible with Graffiti!");
	}
	...
	if (!WantGraffiti && !M_CheckParm("-c2p"))
	{
		if ((CyberGfxBase=OpenLibrary("cybergraphics.library",0)))
		{
			if (InWindow)
			{
				...
				{
					if (IsCyberModeID(smode)) GfxBoard=TRUE;
				}
			} else {
				if (IsCyberModeID(modeid)) GfxBoard=TRUE;
			}
			CloseLibrary(CyberGfxBase);CyberGfxBase=0;
		}
	}
	
	if (!GfxBoard && !InWindow) MakeFlipTask();
	if (!GfxBoard) GetC2P();
	...
	{		
		...
		if (!Planes1Mem) I_Error("I_Video: Can't alloc chip memory for BitMap");

		Planes1=(void *)((ULONG)(Planes1Mem+4095) & (~0xFFF));

		if (DoMMU1) oldmmu_planes1=mmu_mark(Planes1,(REALSCREENWIDTH / 8 * REALSCREENHEIGHT * Depth + 4095) & (~0xFFF),CM_IMPRECISE,SysBase);
		...
		if (DoDoubleBuffer)
		{
			...
			if (!Planes2Mem)
			{
				...
				if (DoMMU1) oldmmu_planes2=mmu_mark(Planes2,(REALSCREENWIDTH / 8 * REALSCREENHEIGHT * Depth + 4095) & (~0xFFF),CM_IMPRECISE,SysBase);
	...
	if (DoMMU1 && screens[0])
	{
		oldmmu_chunky=mmu_mark(screens[0],(REALSCREENWIDTH * REALSCREENHEIGHT + 4095) & (~0xFFF),CM_WRITETHROUGH,SysBase);
		ChunkyMMUed=TRUE;
	}
	
	if (DoMMU2)
	{
		mmu_stuff2();MMU2ed=TRUE;
	}
	...
	if (hidemouse)
	{
		...
		{
			MausObj=NewObject(0,"pointerclass",
				POINTERA_BitMap,(int)MausBM,
				TAG_DONE);
		}
	}

	rect.MinX=0;rect.MaxX=REALSCREENWIDTH-1;
	rect.MinY=0;rect.MaxY=REALSCREENHEIGHT-1;
	...
	if (!InWindow)
	{
		if (!DoGraffiti)
		{
			screen=OpenScreenTags(0,
				SA_Width,REALSCREENWIDTH,
				SA_Height,REALSCREENHEIGHT,
				SA_Depth,8,
				SA_Behind,TRUE,
				SA_Quiet,TRUE,
				SA_DisplayID,modeid,
				DoC2P ? SA_DClip : TAG_IGNORE,(int)&rect,
				DoC2P ? SA_BitMap : TAG_IGNORE,(int)&bitmap1,
				TAG_DONE);
	
			if (!screen) I_Error("Can't open screen!");
		} else {
	...
	if (usemouse) flags |= WFLG_REPORTMOUSE;
	if (InWindow)
	{
	...
	if (usemouse) idcmp |= (IDCMP_MOUSEMOVE|IDCMP_MOUSEBUTTONS|IDCMP_DELTAMOVE);
	if (InWindow) idcmp |= IDCMP_CLOSEWINDOW;

	if (M_CheckParm("-nosleep")) idcmp &= (~(IDCMP_ACTIVEWINDOW|IDCMP_INACTIVEWINDOW));
	
	if (!DoGraffiti)
	{
		window=OpenWindowTags(0,
			InWindow ? WA_PubScreen : WA_CustomScreen,(int)screen,
			InWindow ? WA_Title : TAG_IGNORE,(int)"DoomAttack",
			WA_Left,InWindow ? (screen->Width - REALSCREENWIDTH - screen->WBorLeft - screen->WBorRight) / 2 : 0,
			WA_Top,InWindow ? (screen->Height - REALSCREENHEIGHT - screen->WBorTop - screen->WBorBottom - screen->Font->ta_YSize - 1) / 2 : 0,
			InWindow ? WA_InnerWidth : WA_Width,REALSCREENWIDTH,
			InWindow ? WA_InnerHeight : WA_Height,REALSCREENHEIGHT,
			WA_Flags,flags,
			WA_IDCMP,idcmp,
			WA_AutoAdjust,TRUE,
			MausObj ? WA_Pointer : TAG_IGNORE, (int)MausObj,
			TAG_DONE);
	} else {
		ModifyIDCMP(window,idcmp);
	}
		
	if (!window) I_Error("I_Video: Can't open window!");
	...
	{
		Planes=Planes2;ActBuffer=1;ActScreen=0;
	} else {
		Planes=Planes1;ActBuffer=ActScreen=0;
	}
...
struct InputEvent *C_MyInputHandler(struct InputEvent *ie)
{
	...
	if (window && (IntuitionBase->ActiveWindow == window))
	{
		...
		} else if ((ie->ie_Class==IECLASS_RAWMOUSE) && usemouse && full_mouse && (gamestate==GS_LEVEL) && !paused && !menuactive)
		{
			code=ie->ie_Code;
			ActQualifier=ie->ie_Qualifier;
