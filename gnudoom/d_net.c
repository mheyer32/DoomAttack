...
doomdata_t *netbuffer = NULL; /* points inside doomcom*/
...
extern unsigned SWAPLONG(unsigned a);
...
unsigned NetbufferChecksum (void)
{
   ...
   int		i,l;
    ...
    /* FIXME -endianess?*/
#ifdef NORMALUNIX
	if (!PCCheckSum) return 0;			/* byte order problems*/
#endif
...
static __inline int ExpandTics(int low)
{
    register int _ret __asm("d0");
    register int d0 __asm("d0") = low;
...
void HHSendPacket(int node, int flags)
{
...
static void GetPackets(void)
{
    ...
    while (HGetPacket()) {
        ...
        if (netbuffer->checksum & NCMD_EXIT) {
            ...
            theconsoleplayer->message = exitmsg;
            if (demorecording)
                G_CheckDemoStatus();
            continue;
        }
...
#else
                dest = &netcmds[netconsole][LongRest(nettics[netnode], BACKUPTICS)];
#endif
...
#else
        G_BuildTiccmd(&localcmds[LongRest(maketic, BACKUPTICS)]);
#endif
...
#else
                netbuffer->cmds[j] = localcmds[LongRest((realstart + j), BACKUPTICS)];
#endif
...
/* CheckAbort*/
/**/
extern boolean I_CheckBreak(void);
...
static void D_ArbitrateNetStart(void)
{
    ...
    i = M_CheckParm("-forceversion");
    if (i && i < myargc) {
        fakeversion = atoi(myargv[i + 1]);
    }
    ...
    if (doomcom->consoleplayer) {
        ...
        while (1) {
            ...
            if (!HGetPacket())
                continue;
            if (netbuffer->checksum & NCMD_SETUP) {
                if (netbuffer->player != fakeversion)
