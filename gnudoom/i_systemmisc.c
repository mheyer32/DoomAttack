...
{
    struct TagItem cmtags[] = {VTAG_VIEWPORTEXTRA_GET, 0, TAG_DONE};
    ...
    if (AslBase) {
        if ((scr = LockPubScreen(NULL))) {
            ...
            if ((FileReq = AllocAslRequest(ASL_FileRequest, NULL))) {
                ScreenToFront(scr);
                rc = AslRequestTags(FileReq, ASLFR_TitleText, (int)title, ASLFR_Screen, (int)scr, ASLFR_InitialLeftEdge,
                                    wx, ASLFR_InitialTopEdge, wy, ASLFR_InitialWidth, sx, ASLFR_InitialHeight, sy,
                                    ASLFR_InitialFile, (int)initial, ASLFR_DoPatterns, TRUE, ASLFR_InitialPattern,
                                    (int)"~(#?.info)", ASLFR_DoSaveMode, TRUE, TAG_DONE);
                if (rc) {
    ...
    if (AslBase) {
        if ((req = AllocAslRequest(ASL_ScreenModeRequest, NULL))) {
            if ((scr = LockPubScreen(0))) {
                ...
                rc = BestModeID(BIDTAG_NominalWidth, SCREENWIDTH, BIDTAG_NominalHeight, REALSCREENHEIGHT, BIDTAG_Depth,
                                8, BIDTAG_DIPFMustNotHave, propertymask, TAG_DONE);

                if (AslRequestTags(req, ASLSM_TitleText, (int)title, ASLSM_Screen, (int)scr, ASLSM_InitialLeftEdge, wx,
                                   ASLSM_InitialTopEdge, wy, ASLSM_InitialWidth, sx, ASLSM_InitialHeight, sy,
                                   ASLSM_InitialDisplayID, rc, ASLSM_MinWidth, SCREENWIDTH, ASLSM_MinHeight,
                                   SCREENHEIGHT, ASLSM_MinDepth, 8, ASLSM_MaxDepth, 8, ASLSM_PropertyFlags, 0,
                                   ASLSM_PropertyMask, propertymask, ASLSM_CustomSMList, (int)&mydisplaylist,
                                   TAG_DONE)) {
                    rc = req->sm_DisplayID;
                }
...
void I_IFFScreenShot(char *name, byte *data, int width, int height, byte *palette)
{
    ...
    if (ActQualifier & (IEQUALIFIER_LSHIFT | IEQUALIFIER_RSHIFT)) {
        ...
        if (!(GetFile("Save Screenshot", s, s)))
            return;
        name = s;
    }
