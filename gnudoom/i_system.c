...
/*		if (C2P) C2P->EndChunky2Planar(); ??????:????? */
		...
		if (C2PFile)
		{
			...
			if (!strcmp(id,"C2P"))
			{
				if (C2P->Flags&C2PF_SIGNALFLIP) C2PIsFlipping=TRUE;
				if (C2P->Flags&C2PF_GRAFFITI) DoGraffiti=TRUE;
				...
				if (((REALSCREENHEIGHT != 200) &&
					 (!(C2P->Flags&C2PF_VARIABLEHEIGHT))) ||
					 ((REALSCREENWIDTH != 320) &&
					 (!(C2P->Flags&C2PF_VARIABLEWIDTH))))
				{
					fprintf(stderr,"I_Init: The selected C2P routine does not support %ld x %ld resolutions.\n"
										"        DoomAttack will work in RTG mode!\n",REALSCREENWIDTH,REALSCREENHEIGHT);
					getchar();
				} else {

					if (C2P->InitChunky2Planar(REALSCREENWIDTH,REALSCREENHEIGHT,REALSCREENWIDTH*REALSCREENHEIGHT/8,&init))
					{
						DoC2P=TRUE;
						...
						DoGraffiti=FALSE;
						C2PIsFlipping=FALSE;
						fprintf(stderr,"I_Init: Initialization of Chunky2Planar routine failed!\n"
											"        DoomAttack will work in RTG mode!\n");
						getchar();
					}
...
void I_Init (void)
{
	...
	if (!KeymapBase) KeymapBase=OpenLibrary("keymap.library",36);

	if (!IntuitionBase || !GfxBase || !KeymapBase) I_Error("Can't open library!");
	if (GfxBase->LibNode.lib_Version <39) I_Error("You need Amiga OS3.0!");
	...
	if (!TimerBase) I_Error("Can't open timer.device!");
	...
	if (M_CheckParm("-joypad")) DoJoyPad=TRUE;
...
void I_QuitAmiga(void)
{
	if (DoAnalogJoy)
	{
		...
		DeleteTask(AnalogJoyTask);AnalogJoyTask=0;
	}
	...
	if (InputIO)
	{
		if (InputIO->io_Device) CloseDevice((struct IORequest *)InputIO);
...
void I_InitWBArgs(void)
{
	...
	if (M_CheckParm("-68030")) cputype=68030;
	if (M_CheckParm("-68040")) cputype=68040;
	if (M_CheckParm("-68060")) cputype=68060;
...
/*		"fmove.l	#16,fp6 \n\t"
	...
	if (!IconBase) IconBase=OpenLibrary("icon.library",0);
...
void I_InitConfigArgs(void)
{
	...
	if (StartOptions)
	{
		if (StartOptions[0])
		{
			if (!(sp=malloc(strlen(StartOptions)+2))) goto raus;
			strcpy(sp,StartOptions);

			if (!ArgsAlloced)
			{
				if (!(newargv=malloc(100*sizeof(void *)))) goto raus;
