...
static void IP_InitNetwork (int i)
{
  ...
  fprintf(stderr,"I_Init: Using TCP/IP Network\n");
...
//            ((ULONG *)&sw)[1]);
    ...
    SerialBase = (struct Library *)SER_writelen_io->IOSer.io_Device;

#ifdef PPC
  BeginIOSerialPPC((struct IORequest *)SER_writelen_io);
#else
  BeginIO ((struct IORequest *)SER_writelen_io);
   ...
   SerialBase = (struct Library *)SER_write_io->IOSer.io_Device;

#ifdef PPC
  BeginIOSerialPPC((struct IORequest *)SER_write_io);
#else
...
//              SER_readlen_io->IOSer.io_Error);
    ...
    SerialBase = (struct Library *)SER_read_io->IOSer.io_Device;

#ifdef PPC
  BeginIOSerialPPC((struct IORequest *)SER_read_io);
#else
  ...
  SerialBase = (struct Library *)SER_readlen_io->IOSer.io_Device;

#ifdef PPC
  BeginIOSerialPPC((struct IORequest *)SER_readlen_io);
#else
...
void SER_InitNetwork (int i)
{
  ...
  fprintf(stderr,"I_Init: Using Serial Network\n");
}
