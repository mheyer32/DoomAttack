...
static int cache_chip_data (int id)
{
  ...
  if (c->chip_data != NULL)
  {
    FreeMem (c->chip_data,c->len);c->chip_data = NULL;
  }
...
void I_ShutdownSound(void)
{
  ...
  for (i=0;i<NUM_CHANNELS;i++)
  {
  		if (Audio[i]) DeleteIORequest(Audio[i]);
  		if (AudioMP[i]) DeleteMsgPort(AudioMP[i]);
  }
  ...
  { 
    ...
    if (!S_sfx[i].link)
    {
      if (S_sfx[i].data) FreeMem(S_sfx[i].data,lengths[i]);
    }	
  }
...
{ 
	...
	if (SoundPlugin)
	{
		...
		if (M_CheckParm("-nosound") || (numChannels < 1)) return;
		
		if (M_CheckParm("-cachesound")) CacheSound=TRUE;
...
{
	if (DAM) DAMCall_SetMusicVolume(volume);
}

void I_PlaySong(int handle, int looping)
{
	if (DAM) DAMCall_PlaySong(handle,looping);
}

void I_PauseSong (int handle)
{
	if (DAM) DAMCall_PauseSong(handle);
}

void I_ResumeSong (int handle)
{
	if (DAM) DAMCall_ResumeSong(handle);
}

void I_StopSong(int handle)
{
	if (DAM) DAMCall_StopSong(handle);
}

void I_UnRegisterSong(int handle)
{
	if (DAM) DAMCall_UnRegisterSong(handle);
}
