...
char* gammamsg[5] = {GAMMALVL0, GAMMALVL1, GAMMALVL2, GAMMALVL3, GAMMALVL4};
...
static FAR menuitem_t MainMenu[] = {{1, "M_NGAME", M_NewGame, 'n'},
...
static void M_ReadSaveStrings(void)
{
    ...
    for (i = 0; i < load_end; i++) {
        ...
        if (M_CheckParm("-cdrom"))
            sprintf(name, "S:doomdata/" SAVEGAMENAME "%d.dsg", i);
        else
...
static void M_DrawLoad(void)
{
    ...
    V_DrawPatchDirect3(72, 28, 0, W_CacheLumpName("M_LOADG", PU_CACHE));
    for (i = 0; i < load_end; i++) {
...
static void M_DrawSaveLoadBorder(int x, int y)
{
    ...
    V_DrawPatchDirect3(x - 8, y + 7, 0, W_CacheLumpName("M_LSLEFT", PU_CACHE));

    for (i = 0; i < 24; i++) {
        V_DrawPatchDirect3(x, y + 7, 0, W_CacheLumpName("M_LSCNTR", PU_CACHE));
        x += 8;
    }

    V_DrawPatchDirect3(x, y + 7, 0, W_CacheLumpName("M_LSRGHT", PU_CACHE));
}
...
static void M_LoadSelect(int choice)
{
    ...
    /* hallohallohallo */
    if (M_CheckParm("-cdrom"))
        sprintf(name, "S:doomdata/" SAVEGAMENAME "%d.dsg", choice);
    else
...
static void M_DrawSave(void)
{
    ...
    V_DrawPatchDirect3(72, 28, 0, W_CacheLumpName("M_SAVEG", PU_CACHE));
    for (i = 0; i < load_end; i++) {
        M_DrawSaveLoadBorder(LoadDef.x, LoadDef.y + LINEHEIGHT * i);
        M_WriteText(LoadDef.x, LoadDef.y + LINEHEIGHT * i, savegamestrings[i]);
    }
...
static void M_DrawReadThis1(void)
{
    ...
    switch (gamemode) {
    case commercial:
        V_DrawPatchDirect3(0, 0, 0, W_CacheLumpName("HELP", PU_CACHE));
        break;
    case shareware:
    case registered:
    case retail:
        V_DrawPatchDirect3(0, 0, 0, W_CacheLumpName("HELP1", PU_CACHE));
        break;
    default:
        break;
    }
...
static void M_DrawReadThis2(void)
{
    ...
    case commercial:
        ...
        V_DrawPatchDirect3(0, 0, 0, W_CacheLumpName("CREDIT", PU_CACHE));
        break;
    case shareware:
    case registered:
        V_DrawPatchDirect3(0, 0, 0, W_CacheLumpName("HELP2", PU_CACHE));
        break;
    default:
        break;
    }
...
static void M_DrawSound(void)
{
    V_DrawPatchDirect3(60, 38, 0, W_CacheLumpName("M_SVOL", PU_CACHE));
...
static void M_DrawMainMenu(void)
{
    V_DrawPatchDirect3(94, 2, 0, W_CacheLumpName("M_DOOM", PU_CACHE));
}
...
static void M_DrawNewGame(void)
{
    V_DrawPatchDirect3(96, 14, 0, W_CacheLumpName("M_NEWG", PU_CACHE));
    V_DrawPatchDirect3(54, 38, 0, W_CacheLumpName("M_SKILL", PU_CACHE));
}
...
static void M_DrawEpisode(void)
{
    V_DrawPatchDirect3(54, 38, 0, W_CacheLumpName("M_EPISOD", PU_CACHE));
}
...
static void M_DrawOptions(void)
{
    V_DrawPatchDirect3(108, 15, 0, W_CacheLumpName("M_OPTTTL", PU_CACHE));

    V_DrawPatchDirect3(OptionsDef.x + 175, OptionsDef.y + LINEHEIGHT * detail, 0,
                       W_CacheLumpName(detailNames[detailLevel], PU_CACHE));

    V_DrawPatchDirect3(OptionsDef.x + 120, OptionsDef.y + LINEHEIGHT * messages, 0,
...
static void M_ChangeMessages(int choice)
{
    ...
    if (!showMessages)
        theconsoleplayer->message = MSGOFF;
    else
        theconsoleplayer->message = MSGON;
...
#else
        sprintf(endstring, "%s\n\n%s", endmsg[(LongRest(gametic, (NUM_QUITMESSAGES - 2))) + 1], DOSY);
#endif

    M_StartMessage(special & SPECIALF_NOENDMSG ? DOSY : endstring, M_QuitResponse, true);
}
...
static void M_ChangeDetail(int choice)
{
    ...
    if (!detailLevel)
        theconsoleplayer->message = DETAILHI;
    else
        theconsoleplayer->message = DETAILLO;
}
...
static void M_DrawThermo(int x, int y, int thermWidth, int thermDot)
{
    ...
    xx = x;
    V_DrawPatchDirect3(xx, y, 0, W_CacheLumpName("M_THERML", PU_CACHE));
    xx += 8;
    for (i = 0; i < thermWidth; i++) {
        V_DrawPatchDirect3(xx, y, 0, W_CacheLumpName("M_THERMM", PU_CACHE));
        xx += 8;
    }
    V_DrawPatchDirect3(xx, y, 0, W_CacheLumpName("M_THERMR", PU_CACHE));

    V_DrawPatchDirect3((x + 8) + thermDot * 8, y, 0, W_CacheLumpName("M_THERMO", PU_CACHE));
}

static void M_DrawEmptyCell(menu_t* menu, int item)
{
    V_DrawPatchDirect3(menu->x - 10, menu->y + item * LINEHEIGHT - 1, 0, W_CacheLumpName("M_CELL1", PU_CACHE));
}
...
static void M_WriteText(int x, int y, char* string)
{
    ...
    while (1) {
        ...
        if (cx + w > SCREENWIDTH)
            break;
        V_DrawPatchDirect3(cx, cy, 0, hu_font[c]);
        cx += w;
    }
}
...
boolean M_Responder(event_t* ev)
{
    ...
    if (messageToPrint) {
        if (messageNeedsInput == true && !(ch == ' ' || ch == NOKEY[0] || ch == YESKEY[0] || ch == KEY_ESCAPE))
            return false;
    ...
    if (!menuactive)
        ...
        case KEY_F11: /* gamma toggle*/
            ...
            if (usegamma > 4)
                usegamma = 0;
            theconsoleplayer->message = gammamsg[usegamma];
...
void M_Drawer(void)
{
    ...
    for (i = 0; i < max; i++) {
        if (currentMenu->menuitems[i].name[0])
            V_DrawPatchDirect3(x, y, 0, W_CacheLumpName(currentMenu->menuitems[i].name, PU_CACHE));
        y += LINEHEIGHT;
    }

    /* DRAW SKULL*/
    V_DrawPatchDirect3(x + SKULLXOFF, currentMenu->y - 5 + itemOn * LINEHEIGHT, 0,
