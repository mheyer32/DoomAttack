...
static void F_TextWrite(void)
{
...
#else
    count = LongDiv(finalecount - 10, TEXTSPEED);
#endif
    ...
    for (; count; count--) {
        ...
        if (cx + w > SCREENWIDTH)
            break;
        V_DrawPatch3(cx, cy, 0, hu_font[c]);
        cx += w;
    }
}
...
static void F_CastPrint(char* text)
{
    ...
    while (ch) {
        ...
        w = SHORT(hu_font[c]->width);
        V_DrawPatch3(cx, 180, 0, hu_font[c]);
        cx += w;
    }
}
...
static void F_CastDrawer(void)
{
    ...
    V_DrawPatch3(0, 0, 0, W_CacheLumpName("BOSSBACK", PU_CACHE));
    ...
    if (flip)
        V_DrawPatchFlipped3(160, 170, 0, patch);
    else
        V_DrawPatch3(160, 170, 0, patch);
}
...
static void F_DrawPatchCol(int x, patch_t* patch, int col)
{
    ...
    while (column->topdelta != 0xff) {
        ...
        dest = desttop + column->topdelta * REALSCREENWIDTH;
        count = column->length;
...
static void F_BunnyScroll(void)
{
    ...
    if (finalecount < 1130)
        return;
    if (finalecount < 1180) {
        V_DrawPatch3((SCREENWIDTH - 13 * 8) / 2, (SCREENHEIGHT - 8 * 8) / 2, 0, W_CacheLumpName("END0", PU_CACHE));
        laststage = 0;
        return;
    }
...
#else
    stage = LongDiv(finalecount - 1180, 5);
#endif
    ...
    V_DrawPatch3((SCREENWIDTH - 13 * 8) / 2, (SCREENHEIGHT - 8 * 8) / 2, 0, W_CacheLumpName(name, PU_CACHE));
}
...
void F_Drawer(void)
{
    ...
    if (!finalestage)
        ...
        case 1:
            if (gamemode == retail)
                V_DrawPatch3(0, 0, 0, W_CacheLumpName("CREDIT", PU_CACHE));
            else
                V_DrawPatch3(0, 0, 0, W_CacheLumpName("HELP2", PU_CACHE));
            break;
        case 2:
            V_DrawPatch3(0, 0, 0, W_CacheLumpName("VICTORY2", PU_CACHE));
            break;
        case 3:
            F_BunnyScroll();
            break;
        case 4:
            V_DrawPatch3(0, 0, 0, W_CacheLumpName("ENDPIC", PU_CACHE));
            break;
        }
    }
}
