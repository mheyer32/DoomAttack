...
static int S_AdjustSoundParams(mobj_t* listener, mobj_t* source, int* vol, int* sep, int* pitch);
...
void S_StartSoundAtVolume(void* origin_p, int sfx_id, int volume)
{
    ...
    if (origin && origin != theconsoleplayer->mo) {
        rc = S_AdjustSoundParams(theconsoleplayer->mo, origin, &volume, &sep, &pitch);

        if (origin->x == theconsoleplayer->mo->x && origin->y == theconsoleplayer->mo->y) {
            sep = NORM_SEP;
        }
...
#else
            for (i = first_saw; i != next_saw; i = LongRest((i + 1), 10))
#endif
...
#else
            next_saw = LongRest(next_saw + 1, 10);
            if (next_saw == first_saw)
                first_saw = LongRest(first_saw + 1, 10);
#endif
...
void S_ChangeMusic(int musicnum, int looping)
{
    ...
    music->handle = I_RegisterSong(music->data, musicnum);
...
#else
        *vol = 15 + LongDiv(((snd_SfxVolume - 15) * ((S_CLIPPING_DIST - approx_dist) >> FRACBITS)), S_ATTENUATOR);
#endif
...
#else
        *vol = LongDiv((snd_SfxVolume * ((S_CLIPPING_DIST - approx_dist) >> FRACBITS)), S_ATTENUATOR);
#endif
    }
