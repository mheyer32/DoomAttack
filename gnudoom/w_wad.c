...

{
    ...
    do {
        lump_p--;
    } while ((lump_p != first) && ((lump_p->name1 != v1) || (lump_p->name2 != v2)));

    if (lump_p->name1 == v1 && lump_p->name2 == v2)
        memset(lump_p, 0, sizeof(lumpinfo_t));
}

static void W_Group(char* startname1, char* startname2, char* endname1, char* endname2)
{
#define comparename(eins, zwei, drei)                              \
    ((((eins)->name1 == zwei[0]) && ((eins)->name2 == zwei[1])) || \
...
static void W_GroupPatches(void)
{
    ...
    if (M_CheckParm("-fixsprites"))
        fixsprites = true;
    if (M_CheckParm("-fixflats"))
        fixflats = true;
    if (M_CheckParm("-fixpatches"))
        fixpatches = true;
    if (M_CheckParm("-fixspriterot"))
        fixrot = true;

    if (fixsprites)
        W_Group("S_START", "SS_START", "S_END", "SS_END");
    if (fixflats)
        W_Group("F_START", "FF_START", "F_END", "FF_END");
    if (fixpatches)
        W_Group("P_START", "PP_START", "P_END", "PP_END");
}
...
void* WW_CacheLumpNum(int lump, int tag)
{
    byte* ptr;
