...
enum {ARG_DEVICE,ARG_UNIT,ARG_BAUD,ARG_MODEMINIT,ARG_MODEMEXIT,ARG_PULSE,NUM_ARGS};
			...
			if (!(mem=AllocVec(len+2,MEMF_CLEAR)))
			{
				...
				if (!(MyArgs=AllocDosObject(DOS_RDARGS,0)))
				{
					...
					if (!ReadArgs(ARG_TEMPLATE,Args,MyArgs))
					{
						...
						if (Args[ARG_UNIT]) DEVICEUNIT = *(LONG *)Args[ARG_UNIT];
						if (Args[ARG_BAUD]) DEVICEBAUD = *(LONG *)Args[ARG_BAUD];
						...
						if (Args[ARG_PULSE]) pulsedial=TRUE;
...
static void ModemCommand(char *str)
{
	...
	for (i=0;i<=l;i++)
	{
		...
		{
			Delay(1);
			if (CheckIO((struct IORequest *)SerWriteIO)) break;
...
static void ModemResponse(char *resp)
{
	...
	do
	{
		...
		{ 
			...
			if (!SerReadIO->IOSer.io_Actual) continue;
...
/*===========*/
	...
	if (!(SerWriteMP=CreateMsgPort())) I_Error("DANet_Nullmodem: Can't create Write MsgPort!");
...
void DAN_CleanupNetwork (void)
{
	...
	if (SerWriteIO) DeleteIORequest((struct IORequest *)SerWriteIO);
	if (SerReadIO) DeleteIORequest((struct IORequest *)SerReadIO);
