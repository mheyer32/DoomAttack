				   ...
				   if ((commandwindow=OpenWindowTags (0,
						...
						TAG_DONE)))
					{
						...
						{	
							...
							{
								MouseObj=NewObject(0,"pointerclass",
									POINTERA_BitMap,(int)MouseBM,
									TAG_DONE);
							}
...
#pragma GCC diagnostic pop
	   ...
	   VideoControlTags(commandscreen->ViewPort.ColorMap,
			VTAG_USERCLIP_SET,0,
			TAG_DONE);
...
void Graffiti_Exit(void)
{
	...
	if (screenbuffer[0]) FreeScreenBuffer(screen,screenbuffer[0]);
	if (screenbuffer[1]) FreeScreenBuffer(screen,screenbuffer[1]);

	if (window) CloseWindowSafely(window);
	if (MouseObj) DisposeObject(MouseObj);
	if (MouseBM) FreeBitMap(MouseBM);

	if (screen) CloseScreen(screen);
	
	if (commandwindow) CloseWindowSafely(commandwindow);
	if (commandscreen) CloseScreen(commandscreen);
	
	WaitBlit();
	if (Planes1) FreeMem(Planes1,320 * 222);
	if (Planes2) FreeMem(Planes2,320 * 222);	
