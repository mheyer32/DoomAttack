...
void HUlib_drawTextLine(hu_textline_t* l, boolean drawcursor)
{
    ...
    for (i = 0; i < l->len; i++) {
        ...
        if (c != ' ' && c >= l->sc && c <= '_') {
            ...
            if (x + w > SCREENWIDTH)
                break;
            V_DrawPatchDirect3(x, l->y, FG, l->f[c - l->sc]);
            x += w;
        } else {
            x += 4;
            if (x >= SCREENWIDTH)
    ...
    if (drawcursor && x + SHORT(l->f['_' - l->sc]->width) <= SCREENWIDTH) {
        V_DrawPatchDirect3(x, l->y, FG, l->f['_' - l->sc]);
    }
}
...
void HUlib_eraseTextLine(hu_textline_t* l)
{
    ...
    if ((!automapactive || (hudmap > 0)) && viewwindowx && l->needsupdate) {
        lh = (SHORT(l->f[0]->height) << (medres)) + 1;
        for (y = l->y, yoffset = y * REALSCREENWIDTH; y < l->y + lh; y++, yoffset += REALSCREENWIDTH) {
            if (y < viewwindowy || y >= viewwindowy + viewheight)
                R_VideoErase(yoffset, REALSCREENWIDTH); /* erase entire line*/
            else {
