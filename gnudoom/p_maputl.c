...
/**/
#if 0
static int P_PointOnDivlineSide
( fixed_t	x,
  fixed_t	y,
  divline_t*	line )
{
...
static __inline int P_PointOnDivlineSide(fixed_t x,fixed_t y,divline_t *line)
{
	register int _res __asm("d0");
	
	register fixed_t d0 __asm("d0") = x;
	register fixed_t d1 __asm("d1") = y;
	register divline_t *a0  __asm("a0") = line;
...
/* for all lines.*/
/* */
static boolean P_TraverseIntercepts
( traverser_t	func,
  fixed_t	maxfrac )
{
    int			count;
  ...
  boolean (*trav) (intercept_t *))
{
	...
	ystep = FixedDiv2 (y2-y1,abs(x2-x1));
    }
    else if (xt2 < xt1)
    {
	mapxstep = -1;
	...
	xstep = FixedDiv2 (x2-x1,abs(y2-y1));
    }
    else if (yt2 < yt1)
    {
	mapystep = -1;
